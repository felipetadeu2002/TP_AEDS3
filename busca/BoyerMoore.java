...
public class BoyerMoore {
    ...
    private static int[] construirBadChar(String padrao) {
    ...
    public static ArrayList<Integer> buscar(String texto, String padrao) {
        ...
        int[] badChar = construirBadChar(padrao);
        int m = padrao.length();
        int n = texto.length();
        int deslocamento = 0;
        ...
        while (deslocamento <= (n - m)) {
            ...
            if (j < 0) {
                ocorrencias.add(deslocamento);
                deslocamento += (deslocamento + m < n)? m - badChar[texto.charAt(deslocamento + m)]:1;

            } else {
                deslocamento += Math.max(1, j - badChar[texto.charAt(deslocamento + j)]);
            }
        }
        return ocorrencias;
    }
