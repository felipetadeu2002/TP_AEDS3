...
public class LZW {
    ...
    int codigo = 256;
    String anterior = dicionario.get(comprimido.get(0));
    StringBuilder resultado = new StringBuilder(anterior);

    for (int i = 1; i < comprimido.size(); i++) {
        int atualCodigo = comprimido.get(i);
        String entrada;

        if (dicionario.containsKey(atualCodigo)) {
            entrada = dicionario.get(atualCodigo);
        } else {
    ...
    byte[] bytes = new byte[resultado.length()];
